    ...
    if prompt.startswith("http"):
        ...
        # Otherwise, it's a single track
            ...
            if song_url:
                ...
                await interaction.followup.send(f"Error: Could not retrieve URL for the song {songs.title}.")
...
async def upload_play(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        "Now upload an audio file to play in the voice channel:",
        ephemeral=True)
    ...
    try:
        ...
        song_queue.append({
            'type': 'file',   # Indicate this is a local file
            'title': audio_file.filename,
            'path': audio_file_path  # Include the file path
        })
...
async def stop(interaction: discord.Interaction):
    ...
    if voice_client:
        logger.info(f"Received stop command from {interaction.user}. Stopping playback and disconnecting the bot.")
        ...
        logger.warning(f"Stop command received, but bot is not connected to a voice channel. Command issued by {interaction.user}.")
...
async def queue(interaction: discord.Interaction):
    if len(song_queue) == 0:
        logger.info(f"Queue command issued by {interaction.user}, but the queue is currently empty.")
        ...
        logger.info(f"Queue command issued by {interaction.user}. Current queue:\n{queue_list}")
