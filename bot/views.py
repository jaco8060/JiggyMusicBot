...
from bot.music import YTDLSource, play_next_song
import discord
import asyncio
import logging
    ...
    def create_callback(self, idx):
        async def callback(interaction):
            ...
            try:
                ...
                song = await YTDLSource.from_url(video['url'], loop=self.interaction.client.loop, download=False, max_retries=3, retry_delay=5)

                # Ensure the song is a valid YTDLSource object
                if song is None:
                    await interaction.followup.send(f"Error processing the video.")
                    return
